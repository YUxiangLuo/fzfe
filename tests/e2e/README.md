...
| `bun run e2e:auth` | 认证加固回归：登录限流、自助注册限流、`null` 注册 JSON 400 |
| `bun run e2e:runtime-cleanup` | 运行时磁盘清理回归：启动清理导出、临时上传、guided training 和模型产物 |
...
| `bun run e2e:all` | 运行主线 E2E 套件（auth/runtime-cleanup/teacher/assistant/admin/shiyan/session） |
...
- 常规 Playwright 配置会把认证限流阈值抬高，避免长套件中的重复登录误伤；`e2e:auth` 会单独压低阈值来验证限流本身。
- `e2e:runtime-cleanup` 会用专门启动脚本先写入磁盘清理夹具，再启动真实后端验证 startup cleanup；不要和其他套件并行运行。
