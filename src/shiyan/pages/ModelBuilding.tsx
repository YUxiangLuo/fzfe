...
const ModelBuilding: React.FC<Props> = ({
  appState,
  updateAppState,
  completeStep,
}) => {
  ...
  const getCompanyName = () => {
    ...
    return (
      companyNames[appState.selectedIndustry || ""]?.[
  ...
  const getProductName = () => {
    ...
    return (
      productNames[appState.selectedIndustry || ""]?.[
        appState.selectedCompany || ""
      ]?.[appState.selectedProduct || ""] || "选定产品"
    );
  };
      ...
      <div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-8">
          ...
          <div className="text-blue-700 space-y-2">
            <p>
              • <strong>分析市场需求</strong>：基于{getProductName()}
              的历史销售数据，预测未来6个月的市场需求
            </p>
            <p>
              • <strong>选择预测方法</strong>
              ：从多种预测算法中选择最适合的方法，确保预测准确性
            </p>
            <p>
              • <strong>制定决策建议</strong>
              ：为生产部门提供科学的需求预测，支持生产计划制定
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {models.map((model) => {
              ...
              return (
                <div
                  key={model.id}
                  className="p-6 bg-gray-50 rounded-lg border border-gray-200 h-full"
                >
                  ...
                  <div className="flex items-center space-x-3">
                    <span
                      className={`px-2 py-1 text-xs font-medium rounded-full border ${getDifficultyColor(model.difficulty)}`}
                    >
                      {model.difficulty}
                    </span>
                    <span
                      className={`text-xs font-medium ${getAccuracyColor(model.accuracy)}`}
                    >
                      {model.accuracy}
                    </span>
                  </div>
        ...
        <div className="mt-8 bg-yellow-50 border border-yellow-200 rounded-lg p-6">
          ...
          <div className="text-yellow-700 text-sm space-y-2">
            <p>
              • <strong>初次使用建议</strong>
              ：如果您是第一次进行需求预测，建议从移动平均法开始，它简单易懂且计算快速
            </p>
            <p>
              • <strong>追求高精度</strong>
              ：如果您希望获得最准确的预测结果，LSTM神经网络通常能提供最佳性能
            </p>
            <p>
              • <strong>平衡考虑</strong>
              ：ARIMA模型在准确性和可解释性之间提供了良好的平衡，适合向上级汇报
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="w-full">
        <div className="mb-8">
          ...
          <p className="text-lg text-gray-600">
            选择合适的预测算法来分析产品的需求。每种方法都有其特点和适用场景。
          </p>
        </div>
          ...
          <div className="w-80 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <nav className="space-y-2">
              {models.map((model) => {
                const Icon = model.icon;
                const isActive =
                  location.pathname === `/model/${model.path}` || false;
