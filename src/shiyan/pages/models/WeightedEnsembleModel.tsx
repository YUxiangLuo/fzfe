...
import { BarChart3, Settings, Play, Layers, CheckCircle, Award, X, History } from 'lucide-react';
...
const WeightedEnsembleModel: React.FC<Props> = ({ completeStep }) => {
  ...
  // 模拟加权融合预测结果
  const predictions = [
    { month: '2025-01', predicted: 1905, ma: 1850, es: 1890, arima: 1880, lstm: 1920 },
    { month: '2025-02', predicted: 2290, ma: 2200, es: 2250, arima: 2280, lstm: 2350 },
    { month: '2025-03', predicted: 1835, ma: 1920, es: 1840, arima: 1820, lstm: 1850 },
    { month: '2025-04', predicted: 1765, ma: 1750, es: 1770, arima: 1750, lstm: 1780 },
    { month: '2025-05', predicted: 2135, ma: 2100, es: 2120, arima: 2150, lstm: 2180 },
    { month: '2025-06', predicted: 2465, ma: 2400, es: 2420, arima: 2450, lstm: 2520 },
  ];
    ...
    <div className="h-full">
          ...
          <div className="bg-white rounded-2xl max-w-6xl w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between rounded-t-2xl">
              <div className="flex items-center space-x-3">
                ...
                <h2 className="text-xl font-semibold text-green-800">加权平均融合模型训练完成</h2>
              </div>
              <button
            ...
            <div className="p-6 space-y-6">
                    ...
                    {predictions.map((pred, index) => (
                      <div key={index} className="p-3 bg-white rounded-lg">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <div className="text-indigo-600 font-semibold">{pred.predicted.toLocaleString()}件</div>
                        </div>
                ...
                <div className="bg-gray-50 rounded-xl p-6">
                  ...
                  <div className="space-y-4">
                    <div className="p-4 bg-indigo-50 border border-indigo-200 rounded-lg">
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="font-semibold text-indigo-800">MAPE (平均绝对百分比误差)</h4>
                        <Award className="w-5 h-5 text-indigo-600" />
                      </div>
                      <div className="text-2xl font-bold text-indigo-600">{evaluationMetrics.mape}%</div>
                      <p className="text-sm text-indigo-700">优秀！融合模型显著提升精度</p>
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-3 bg-white rounded-lg">
                        <div className="text-sm text-indigo-600">RMSE</div>
                        <div className="text-lg font-bold text-indigo-900">{evaluationMetrics.rmse}</div>
                      </div>
                      <div className="p-3 bg-white rounded-lg">
                        <div className="text-sm text-indigo-600">R²</div>
                        <div className="text-lg font-bold text-indigo-900">{evaluationMetrics.r2}</div>
                      </div>
                    </div>

                    <div className="p-3 bg-yellow-50 rounded-lg">
                      <div className="text-sm text-yellow-600">最优权重组合</div>
              ...
              <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-6">
                <h4 className="font-semibold text-indigo-800 mb-3">加权融合模型优势分析</h4>
      ...
      <div className="mb-6">
        <div className="flex items-center space-x-3 mb-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">加权平均融合模型</h1>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                <div className="text-sm text-gray-600 mb-2">权重总和: {((maWeight + esWeight + arimaWeight + lstmWeight) * 100).toFixed(1)}%</div>
                <div className="w-full bg-gray-200 rounded-full h-3">
                  <div className="flex h-3 rounded-full overflow-hidden">
                    <div className="bg-blue-500" style={{ width: `${maWeight * 100}%` }}></div>
                    <div className="bg-orange-500" style={{ width: `${esWeight * 100}%` }}></div>
                    <div className="bg-green-500" style={{ width: `${arimaWeight * 100}%` }}></div>
                    <div className="bg-purple-500" style={{ width: `${lstmWeight * 100}%` }}></div>
                  </div>
                </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="bg-gray-50 rounded-lg p-6">
              <div className="grid grid-cols-4 gap-4 mb-6">
                {[
                  { name: 'MA', color: 'bg-blue-100 text-blue-600', weight: maWeight },
                  { name: 'ES', color: 'bg-orange-100 text-orange-600', weight: esWeight },
                  { name: 'ARIMA', color: 'bg-green-100 text-green-600', weight: arimaWeight },
                  { name: 'LSTM', color: 'bg-purple-100 text-purple-600', weight: lstmWeight }
                ].map((model, index) => (
                  <div key={index} className="text-center">
                    <div className={`w-16 h-16 ${model.color} rounded-lg flex items-center justify-center mx-auto mb-2`}>
                      <span className="text-sm font-bold">{model.name}</span>
                    </div>
                    <div className="text-xs text-gray-600">{(model.weight * 100).toFixed(1)}%</div>
                  </div>
                ))}
              </div>
              ...
              <div className="flex items-center justify-center mb-4">
                <div className="text-center">
                  <div className="w-20 h-20 bg-indigo-100 rounded-lg flex items-center justify-center mx-auto mb-2">
                    <span className="text-sm font-bold text-indigo-600">融合</span>
                  </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            {isTraining && (
              <div className="mt-4">
                ...
                <p className="text-sm text-gray-600 mt-2 text-center">正在优化权重组合...</p>
              </div>
            )}
          </div>

          <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-6">
            <h3 className="text-lg font-semibold text-indigo-800 mb-3">融合原理</h3>
            ...
            <div>
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{(lastPrediction.maWeight * 100).toFixed(1)}%</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">指数平滑法:</span>
                  <span className="font-medium">{(lastPrediction.esWeight * 100).toFixed(1)}%</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">ARIMA模型:</span>
                  <span className="font-medium">{(lastPrediction.arimaWeight * 100).toFixed(1)}%</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">LSTM神经网络:</span>
                  <span className="font-medium">{(lastPrediction.lstmWeight * 100).toFixed(1)}%</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">优化方法:</span>
                  <span className="font-medium">{lastPrediction.optimizationMethod.toUpperCase()}</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-4 text-sm">
                <div className="text-center p-2 bg-indigo-50 rounded">
                  <div className="font-bold text-indigo-600">{lastPrediction.metrics.mape}%</div>
