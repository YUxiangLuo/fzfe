...
const GradesOverview: React.FC = () => {
  ...
  // 计算统计数据
  const averageScore = Math.round(grades.reduce((sum, grade) => sum + grade.totalScore, 0) / grades.length * 10) / 10;
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{highScoreCount}</p>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-xs text-orange-600">{passCount}/{grades.length}</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-lg shadow-md p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* 各班级平均分 */}
        <div className="bg-white rounded-lg shadow-md p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">各班级平均分</h3>
              ...
              <div key={index} className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center space-x-2">
                    <span className="text-lg font-bold text-blue-600">{cls.average}</span>
                    <span className="text-xs text-gray-500">({cls.count}人)</span>
                  </div>
                </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="space-y-4">
            {[
              { label: '优秀 (90-100)', count: scoreDistribution.excellent, color: 'bg-green-500' },
              { label: '良好 (80-89)', count: scoreDistribution.good, color: 'bg-blue-500' },
              { label: '中等 (70-79)', count: scoreDistribution.average, color: 'bg-yellow-500' },
              { label: '及格 (60-69)', count: scoreDistribution.pass, color: 'bg-orange-500' },
              { label: '不及格 (<60)', count: scoreDistribution.fail, color: 'bg-red-500' }
            ].map((item, index) => (
      ...
      <div className="bg-white rounded-lg shadow-md">
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-gray-900">学生成绩详单</h2>
            <div>
              <select
                value={selectedClass}
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">实验流程</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">知识测试</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">模型选择</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">实验报告</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {grades.map((grade, index) => (
                <tr key={grade.studentId} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    <span className={`font-medium ${getScoreColor(grade.experimentProcess)}`}>
                      {grade.experimentProcess}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    <span className={`font-medium ${getScoreColor(grade.knowledgeTest)}`}>
                      {grade.knowledgeTest}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    <span className={`font-medium ${getScoreColor(grade.modelSelection)}`}>
                      {grade.modelSelection}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    <span className={`font-medium ${getScoreColor(grade.experimentReport)}`}>
                      {grade.experimentReport}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <span className={`font-bold text-lg ${getScoreColor(grade.totalScore)}`}>
                      {grade.totalScore}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 py-1 text-xs rounded-full font-medium ${getScoreBgColor(grade.totalScore)} ${getScoreColor(grade.totalScore)}`}>
