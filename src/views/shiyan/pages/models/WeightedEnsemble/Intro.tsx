  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-6 bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg border border-purple-200 shadow-sm">
        <h4 className="text-lg font-semibold text-gray-800 mb-3">合并滚动OOF残差MSE倒数加权法</h4>
        <p className="text-gray-800 leading-relaxed text-base mb-4">
          MSE倒数加权是一种常见的组合预测权重确定方法。它借鉴逆方差加权思想；本系统先合并最多三折时间因果滚动OOF残差，再依据残差均方误差确定候选权重，即对MSE较小的模型赋以较高权重。
        </p>
      ...
      <div className="p-6 bg-gradient-to-br from-blue-50 to-cyan-50 rounded-lg border border-blue-200 shadow-sm">
        ...
        <div className="space-y-4 text-gray-800 leading-relaxed text-base">
          <div>
            <p className="mb-2">其中，MSE<sub>i</sub> 是模型 i 在全部有效滚动折合并OOF样本上的残差均方误差：</p>
      ...
      <div className="p-5 bg-green-50 rounded-lg border border-green-200">
        ...
        <div className="space-y-2 text-gray-700 leading-relaxed text-base">
          <p>
            <strong>训练期数据驱动权重：</strong>系统根据最多三折扩展窗口滚动起点OOF残差的合并MSE计算一次权重；产物用于预测时权重保持固定，只有重新训练才会更新。
          </p>
          <p>
      ...
      <div className="p-5 bg-sky-50 rounded-lg border border-sky-200">
        ...
        <p className="text-gray-700 leading-relaxed text-base mb-3">
          系统按模型数、样本量和所有成员的可训练条件生成最多三折、验证段互不重叠的扩展窗口滚动起点预测；样本不足时明确退化为单折。先用合并OOF残差中带数据尺度 ε 的 1/MSE 得到候选权重，再按OOF点数可靠度向等权组合收缩，避免短校准样本把权重几乎全押在单一成员。本实现仍不估计成员误差协方差。
        </p>
        <p className="text-gray-700 leading-relaxed text-base mb-3">
          引导训练把每个“基础模型 × 滚动折”作为可单独执行和失败重试的检查点。单折完成只保存该折OOF预测，不会提前生成最终权重；全部有效折完成后才统一合并残差、计算并收缩权重。预留但不存在的折会明确标记为已跳过。
        </p>
        <p className="text-gray-700 leading-relaxed text-base">
          滚动验证只继承用户选择的 MA 窗口、ES α、ARIMA d 与 LSTM 历史特征/归一化。ARIMA 阶数和 LSTM 的 look-back、容量、批大小、轮数都按每折当前前缀重新推导，避免未来信息泄漏；LSTM 不接收任何已知未来特征。不确定性仅用同一批OOF组合残差校准成员逐 horizon 增长形状，独立评估真实值不参与校准；证据不足时会明确标记 fallback。
        </p>
      </div>
    </div>
  );
