...
export const TRAINING_PROGRESS_PROFILES: Record<ModelProgressProfileKey, TrainingProgressProfile> = {
  ...
  ensemble: {
    ...
    steps: [
      ...
      { label: '生成基础预测', description: '按后端阶段计划训练或复用基础模型；滚动OOF任务会逐模型、逐折执行。', weight: 3.8 },
  ...
  weighted_avg: {
    title: '加权平均融合训练中',
    subtitle: '后端正在基于滚动起点OOF残差计算基础模型权重并生成加权预测。',
    estimate: '可能需要几分钟',
    accent: 'purple',
    steps: [
      { label: '检查基础模型', description: '只读取用户选择的成员配置；隐藏参数将在当前数据段重算。', weight: 1.0 },
      { label: '生成滚动OOF预测', description: '最多三折扩展历史前缀；每个成员×折独立执行检查点，ARIMA阶数与LSTM配置逐折重新推导。', weight: 3.0 },
      { label: '计算模型权重', description: '按合并OOF残差MSE倒数计算候选权重，再依据样本可靠度向等权收缩。', weight: 1.3 },
      { label: '完整训练与加权组合', description: '成员销量先截断再加权；独立评估只计算指标，不确定性由滚动OOF组合残差校准。', weight: 2.5 },
      ...BASE_FINAL_STEPS,
    ],
  ...
  stacking: {
    ...
    steps: [
      ...
      { label: '生成 Level-1 OOF', description: '最多三折扩展Level-0前缀，每个成员×折独立执行检查点并预测互不重叠的Level-1区间。', weight: 3.3 },
      { label: '训练元模型', description: '全部有效折完成后拼接OOF矩阵，求解一次非负无截距最小二乘（NNLS），保留未归一化系数。', weight: 1.4 },
      { label: '生成最终评估预测', description: '截断成员销量后交给元模型；不确定性沿用合并Level-1 OOF组合残差校准。', weight: 2.6 },
      ...BASE_FINAL_STEPS,
    ],
  ...
  boosting: {
    ...
    accent: 'amber',
    steps: [
      { label: '检查基础模型', description: '读取用户选择的成员配置，并按数据推导最多三折滚动起点及最大轮数。', weight: 1.0 },
      { label: '初始化OOF残差', description: '逐折只使用历史前缀，并以合并OOF原始销量作为第一轮残差目标。', weight: 1.0 },
      { label: '逐轮残差学习', description: '每轮按候选模型×滚动折独立执行检查点；首轮拟合销量，后续拟合有正有负的当前残差。', weight: 4.2 },
      { label: '组合模型链', description: '每轮全部有效折完成后才用合并OOF非负平方损失线搜索求系数并选择胜者；全量重训保留该系数。', weight: 2.0 },
      ...BASE_FINAL_STEPS,
    ],
