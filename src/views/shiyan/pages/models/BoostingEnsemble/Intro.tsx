    ...
    <div className="space-y-6">
      ...
      <div className="p-5 bg-sky-50 rounded-lg border border-sky-200">
        ...
        <p className="mt-3 text-gray-700 leading-relaxed text-base">
          名义 95% 范围与名义 99% 上侧误差复用同一个 Boosting 选模时间留出段的一次固定预测原点跨 horizon 组合残差，并据此估计各阶段逐 horizon 增长形状；由于该留出段还用于候选、系数和轮数选择，这些范围明确标记为未校准、无覆盖率保证。独立评估区间的真实值只用于误差指标，不参与误差范围估计；最终销量区间同步限制为非负并保证包含点预测。
        </p>
      </div>
    </div>
  );
