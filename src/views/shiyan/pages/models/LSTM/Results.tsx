...
const Results: React.FC<ResultsProps> = ({ data, isLoading, error, onRetry, currentProgress, progressEvents }) => {
  ...
  const status = <CalculationStatus isLoading={isLoading} error={error} onRetry={onRetry} modelType="lstm" currentProgress={currentProgress} progressEvents={progressEvents} />;
  if (isLoading || error) {
    return status;
  }
