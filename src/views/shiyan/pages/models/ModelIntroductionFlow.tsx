...
const baseModels: Model[] = [
  ...
  },
  {
    ...
    implementationNotes: [
      ...
      'statsmodels 会根据训练序列开头的观测自动初始化：短序列以首个观测为起点，较长序列使用开头一段观测计算启发式初值；用户只需指定 α，无需手动设定初始值。',
  ...
  },
  {
    ...
    implementationNotes: [
      ...
      '训练页只开放归一化方式和输入特征选择；Lookback窗口和隐藏单元数由系统按训练样本量自动设置，最大训练轮数默认 20 并支持早停。',
      '训练评估与生产预测都采用直接多步模型；生产阶段固定输出未来 6 期，数据不足时会要求调整窗口，不会静默切换预测策略。',
    ...
    workflow: {
      steps: [
        ...
        { title: '深度建模', description: '构建单层LSTM网络加全连接输出层，配合Dropout与L2正则，使用Adam优化器、学习率衰减和早停策略进行训练。', tip: '回看窗口和隐藏单元数按样本量设置；最大训练轮数默认 20' },
        { title: '多步预测', description: '直接输出未来多个时间步的销量预测，减少递归预测的误差累积。', tip: '生产阶段固定直接输出未来 6 期；数据不足时需先调整窗口' }
      ]
    },
    parameters: [
