...
const baseModels: Model[] = [
  ...
  },
  {
    ...
    implementationNotes: [
      ...
      '标准模式按时间顺序留出验证窗口并隔离标签重叠，EarlyStopping 监控验证损失；选定轮数后在全部窗口重拟合。窗口不足时进入明确标注的教学演示模式。',
      '线性Dense输出层不保证非负；系统在逆缩放后把最终销量点预测按 max(0, ŷ) 截断，并以截断值计算评估残差和指标。std_dev、95%区间和99%上侧误差优先来自临时时间验证模型中逆缩放、未做非负截断的逐 horizon actual-prediction 残差；区间使用经验分位数保留稳定偏差，但由于最终模型会重拟合，仍不是覆盖率经过严格证明的概率区间。Boosting内部拟合有符号残差时会显式关闭点预测截断。'
    ],
    mathematics: {
    ...
    parameters: [
      ...
      { name: '训练轮数 Epochs（系统动态设置）', description: '整个数据集被训练的次数', impact: '标准模式用时间验证早停选取，并在全部窗口按最佳轮数重拟合；演示模式按更新步预算设置。', typical: '由样本窗数和批大小决定，不使用固定默认值' },
