...
const baseModels: Model[] = [
  ...
  },
  {
    ...
    implementationNotes: [
      ...
      '95%预测区间先由原始ARIMA预测分布给出，再将销量域的上下分位数按max(0, 分位数值)映射为非负；std_dev与99%上侧误差仍按未截断分布计算。该映射不是重新拟合截断正态分布，并可能缩窄页面展示的区间。'
    ],
    mathematics: {
    ...
    workflow: {
      steps: [
        ...
        { title: '预测与区间', description: '生成多步预测，将销量点预测与95%区间上下分位数映射为非负；std_dev和99%上侧误差仍来自原始ARIMA分布。', tip: '分位数映射可能缩窄展示区间，但不是重新拟合截断正态分布' }
      ]
    },
    parameters: [
