...
const baseModels: Model[] = [
  {
    ...
    implementationNotes: [
      ...
      '最终作为销量使用的点预测按 max(0, ŷ) 截断，并以截断值计算残差、std_dev和评估指标；这是业务约束，不属于教科书移动平均公式。'
    ],
    mathematics: {
  ...
  },
  {
    ...
    implementationNotes: [
      ...
      '系统明确采用第一个训练观测初始化水平，即 S₁=Y₁；随后按递推公式更新。',
      ...
      '非负训练销量通常会自然得到非负水平；系统仍将最终销量点预测按 max(0, ŷ) 截断，并以截断值计算残差、std_dev和指标。该兜底不是 SES 递推式的一部分。'
    ],
    mathematics: {
    ...
    workflow: {
      steps: [
        ...
        { title: '初始化平滑值', description: '教科书有多种初始化方案；本系统固定取第一个训练观测，即S₁=Y₁。', tip: '明确初始化可让公式与代码逐项复现' },
  ...
  },
  {
    ...
    icon: Sigma,
    category: '高级统计方法',
    summary: 'ARIMA（自回归积分移动平均模型）是经典线性时间序列方法。本系统实现非季节 ARIMA：用户固定差分阶数 d，系统在样本量自适应的边界内分别执行 AIC 与 BIC 的 stepwise 搜索，再从两个优胜模型中选定 p、q。',
    principle: {
      description: 'ARIMA通过三个核心组件建模：AR（自回归）利用差分后序列的历史值；I（差分）处理非平稳性；MA（移动平均）利用历史创新项。训练点少于30时，本系统在两次搜索的优胜者之间按BIC优先选择；至少30点时按AIC优先。',
      keyIdea: '先用固定阶数的非季节差分处理非平稳性，再用 AR 与 MA 项刻画差分后序列的线性相关结构。'
    },
    implementationNotes: [
      '教科书 ARIMA 通常通过平稳性检验、ACF/PACF 图和信息准则共同确定 p、d、q。',
      '本系统由用户指定差分阶数 d；对 p、q 分别运行 AIC 和 BIC 目标的 stepwise 搜索。stepwise 是启发式逐步搜索，不等同于穷举全部组合。',
      '搜索边界按训练点数收紧：不多于12点时p/q上限为1，13–20点时为2，更长时最高为min(5, floor(n/6))。',
      '本模型是非季节 ARIMA，不包含季节差分或季节 AR/MA 项。原始点预测可能为负，系统将用于残差、指标和输出的销量点预测按 max(0, ŷ) 截断。',
      '95%预测区间仍来自未截断的ARIMA预测分布；系统以区间宽度除以3.92近似std_dev，所以这不是截断正态区间。ARIMA区间会随步长展宽，与其他模型的常数残差std_dev口径不同。'
    ],
    mathematics: {
      ...
      example: '若用户设定d=1，AIC与BIC的stepwise搜索可能分别返回不同阶数；系统再按样本量对应的优先准则比较这两个结果，而不是保证检查范围内每个(p,q)。'
    },
    workflow: {
      steps: [
        { title: '平稳性处理', description: '系统对d=0、1、2分别进行ADF检验，用户据此固定本次建模的差分阶数d。', tip: '本系统以p<0.05判为拒绝单位根原假设' },
        { title: '两次逐步搜索', description: '系统按训练点数设置p/q上限，分别运行AIC目标和BIC目标的非季节stepwise搜索。', tip: '逐步搜索不保证遍历所有组合' },
        { title: '优胜者比较', description: '只比较两次搜索各自的优胜者：n<30按BIC优先，否则按AIC优先。', tip: '另一信息准则用于同值时辅助选择' },
    ...
    useCases: [
      ...
      { industry: '制造', scenario: '物料消耗', description: '若原序列通过当前ADF门槛，可设d=0，并比较系统按AIC、BIC搜索得到的非季节ARMA候选模型。' },
      { industry: '销售', scenario: '季度业绩', description: '系统会随小样本收紧p、q上限并优先比较BIC，但信息准则和stepwise搜索只能降低、不能消除过拟合风险。' }
    ],
    pros: [
      { title: '辅助定阶', description: '用两次stepwise搜索降低人工查看ACF/PACF的操作负担，但不保证全局最优' },
      { title: '小样本收缩', description: '样本少时收紧阶数边界并优先比较BIC，以降低而非消除过拟合风险' },
  ...
  },
  {
    ...
    icon: BrainCircuit,
    category: '深度学习方法',
    summary: 'LSTM（长短期记忆网络）用门控循环结构学习时序依赖。本系统把训练截止点之前的多变量历史窗口编码后输入两层LSTM，并由一个直接多步输出头一次预测整个未来跨度。',
    principle: {
    ...
    implementationNotes: [
      ...
      '线性Dense输出层不保证非负；系统在逆缩放后把最终销量点预测按 max(0, ŷ) 截断，并以截断值计算残差、std_dev和指标。Boosting内部拟合有符号残差时会显式关闭这一截断。'
    ],
    mathematics: {
    ...
    workflow: {
      steps: [
        ...
        { title: '多步预测', description: '仅取截止点前最后look_back行，一次输出未来多期，逆缩放后将销量点预测截断为不小于0。', tip: '不会读取未来特征路径；截断是输出业务约束' }
      ]
    },
    parameters: [
      ...
      { name: '归一化方式', description: '数值特征的缩放方法；类别字段始终One-Hot编码', impact: 'MinMax按训练区间最小/最大值线性缩放，Z-score按训练均值和标准差缩放；两者都受异常值影响，且Z-score不会把非正态数据变成正态分布。', typical: '默认MinMax；应结合训练稳定性回测选择' }
    ],
    suitability: {
    ...
    useCases: [
      ...
      { industry: '交通', scenario: '流量预测', description: '利用过去24小时流量预测未来1小时；若历史窗口与样本足够，LSTM可能学到早晚高峰等重复模式。' }
    ],
    pros: [
