...
const Results: React.FC<ResultsProps> = ({ data, isLoading, error }) => {
  ...
  return (
    <div>
      <h3 className="text-lg font-semibold mb-4">Stacking 融合 - 计算结果</h3>
      
      <div className="mb-6 p-4 bg-gray-50 rounded-lg border">
        <h4 className="font-semibold mb-2">元模型（Meta-Model）详情</h4>
        <div className="text-sm space-y-1">
          <p><strong>截距 (Intercept):</strong> {data.meta_model.intercept.toFixed(4)}</p>
          <p><strong>系数 (Coefficients):</strong> {data.meta_model.coefficients.map(c => c.toFixed(4)).join(', ')}</p>
        </div>
      </div>
