...
const StackingEnsembleStepper: React.FC = () => {
  ...
  const handleCalculate = useCallback(async () => {
    ...
    try {
      ...
      const response = await apiClient.post<any>("/models/stacking/training", requestBody);
      ...
      if (response.status === "success") {
        ...
        setResults({
          ...
          meta_model: apiResults.meta_model,
        });

        await updateState({
          ensemble_stacking_base_models: selectedModels,
          ensemble_stacking_metrics_rmse: apiResults.metrics.rmse,
          ensemble_stacking_metrics_mae: apiResults.metrics.mae,
          ensemble_stacking_metrics_r2: apiResults.metrics.r2,
        });
      } else {
  ...
  const handleNext = async () => {
    ...
    if (currentStepIndex === STEPS.length - 1) {
      await updateState({ ensemble_stacking_completed: true });
