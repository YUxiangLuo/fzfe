...
const ensembleModels: EnsembleModel[] = [
  {
    ...
    icon: Scale,
    category: '模型融合方法',
    summary: '加权平均融合汇总基础模型在最多三折扩展窗口滚动起点验证中的折外（OOF）残差，用MSE倒数形成候选权重，再按OOF样本可靠度向等权组合收缩；最终预测是成员预测的固定加权和。',
    principle: {
      description: '加权平均的核心思想是根据时间因果的滚动起点折外误差分配成员贡献。合并OOF残差MSE较小的成员通常获得更高候选权重，最终权重还会向等权组合收缩；组合是否优于单模型必须由独立评估确认。',
      keyIdea: '用数据驱动的方式自动确定各模型的重要性，而非简单平均或主观判断。本系统使用各时间因果滚动折合并后的OOF残差均方误差倒数加权。'
    },
    implementationNotes: [
      '教科书组合预测常用逆方差或误差方差-协方差思想分配权重，核心思想是误差越稳定、越小的模型权重越高。',
      '本系统按模型数、样本量和成员可训练条件生成最多三折、验证段互不重叠的扩展窗口滚动起点预测；样本不足时显式退化为单折。',
      '引导训练把每个“基础模型×滚动折”作为可单独执行、失败重试的检查点；这只改变执行与断点粒度，权重仍在全部有效折完成后按合并OOF残差统一计算。',
      '将各折OOF残差合并后，先按带数据尺度ε的MSE倒数归一化，再按OOF点数可靠度向等权组合收缩，降低短校准样本产生极端权重的风险。',
      '内部验证只继承MA窗口、ES α、ARIMA d和LSTM历史特征/数值缩放方式；ARIMA阶数与LSTM隐藏参数按当前前缀重新推导，LSTM始终不接收已知未来特征。',
      '权重非负且和为1，但本实现不估计成员误差协方差，因此不是完整的最小方差组合。',
      '不确定性只用同一批滚动起点OOF组合残差校准，并继承成员逐horizon的增长形状；独立评估真实值不参与校准，证据不足时明确标记fallback。'
    ],
    mathematics: {
      description: '合并滚动OOF残差MSE倒数加权的计算公式：',
      formula: 'MSE_i = (1/n)Σ(y_t - ŷ_{i,t})²\nv_i = [1/(MSE_i + ε)] / Σ[1/(MSE_j + ε)]\nw_i = ρv_i + (1-ρ)/K, ρ=n/(n+K+1)\nŷ = Σ(w_i · ŷ_i)',
      variables: [
        { symbol: 'w_i', meaning: '模型i的权重' },
        { symbol: 'MSE_i', meaning: '模型i在合并滚动OOF样本上的残差均方误差' },
        { symbol: 'ε', meaning: '防止除以0的极小常数' },
        { symbol: 'ŷ_i', meaning: '模型i的预测值' },
        { symbol: 'ŷ', meaning: '最终融合预测值' }
      ],
      example: '假设3个模型的合并OOF残差MSE为100、200、400，倒数归一化的候选权重约为0.57、0.29、0.14；最终还会按OOF点数向等权重0.33、0.33、0.33收缩。'
    },
    workflow: {
      steps: [
        { title: '训练基础模型', description: '分别训练多个不同的基础模型（如MA、ES、ARIMA、LSTM）。', tip: '结构差异可能带来互补误差，但增加模型不保证改善留出误差' },
        { title: '滚动OOF评估', description: '每个模型在最多三个逐步扩展的历史前缀训练，再预测各自不重叠的后续时间段并合并MSE。', tip: '每折必须同时满足所有成员的最小训练条件，数据不足时退化为单折' },
    ...
    parameters: [{ name: '滚动起点划分（系统动态）', description: '用于计算权重的时间顺序OOF折', impact: '系统综合模型数、样本量和所有成员的可训练性决定；最多三折且验证段互不重叠，当前训练页不开放手动设置', typical: '每折期望约20%，不足时减少折数或为成员可训练性缩减' }],
    suitability: {
      suitable: ['有多个基础模型可用', '基础模型性能有差异', '追求稳健性和鲁棒性', '需要可解释的融合方法', '计算资源有限'],
      notSuitable: ['只有一个基础模型', '所有模型高度相关（预测几乎相同）', '滚动OOF样本不足', '需要捕捉模型间复杂交互', '基础模型都表现很差']
    },
    useCases: [
    ...
    pros: [
      ...
      { title: '误差驱动', description: '权重直接来自合并滚动OOF预测误差，便于解释和复核' }
    ],
    cons: [
      { title: '线性限制', description: '只是简单加权，无法学习模型间非线性关系' },
      { title: '静态权重', description: '权重固定，无法适应数据分布变化' },
      { title: '依赖有限OOF样本', description: '滚动OOF样本不代表未来分布时权重可能失效' },
      { title: '提升有限', description: '当基础模型已很好或很差时，改进空间小' }
    ],
    bestPractices: ['选择差异化的基础模型，避免高度相关', '滚动OOF样本应尽可能代表未来数据分布', '定期用新数据重新计算权重', '权重明显集中于单一成员时，检查OOF样本是否充分及成员误差是否稳定', '对比简单平均，确认加权是否确实带来提升', '当前系统已向等权收缩，仍需结合独立评估解释权重'],
    performance: {
      speed: { level: 'high', description: '融合预测极快；训练阶段需要逐模型、逐折生成最多三折OOF预测' },
      accuracy: { level: 'medium', description: '有互补成员和代表性滚动OOF样本时可能改进，不保证优于最佳单模型' },
      dataRequirement: { level: 'medium', description: '需要足够的时间顺序OOF样本评估误差' },
  ...
  },
  {
    ...
    principle: {
      ...
      keyIdea: '贪心策略 + 残差学习。每一步都依据合并的滚动起点OOF残差选择局部最优异构模型来修正已有误差。'
    },
    implementationNotes: [
      ...
      '每一轮会在全部候选模型中选择最能降低最多三折滚动起点合并OOF残差RMSE的模型；同一种候选类型可在不同轮次重复入选，样本不足时退化为单折。',
      '引导训练把每个“候选模型×滚动折”作为独立检查点；单折完成时不提前计算阶段系数，只有本轮全部有效折完成后才按同一合并OOF线搜索并选择胜者。',
      ...
      '每个候选通过合并OOF平方损失的非负线搜索求阶段系数；短校准样本要求至少5%相对改善，其余要求1%。',
      '最大轮数为min(2K,OOF点数-1)，选定链在完整训练段逐级重训基础学习器，但保留OOF选出的阶段系数，不用全量样本内拟合重新改写部署系数。',
      '不确定性只用滚动起点OOF组合残差校准各阶段逐horizon增长形状；独立评估真实值不参与校准。'
    ],
    mathematics: {
    ...
    cons: [
      ...
      { title: '贪心路径依赖', description: '每轮只按当前候选和合并滚动OOF残差选择局部优胜者，早期选择会影响后续模型链' },
      { title: '验证选择敏感', description: '模型链、动态系数和轮数依赖有限的滚动起点OOF样本，短数据时仍可能过拟合' }
    ],
    bestPractices: ['基模型应包含多种类型（如ARIMA+LSTM）以实现优势互补', '查看每轮动态系数与相对改善，警惕系数极端或改善仅略过阈值', '关注合并滚动OOF误差，系统会在没有候选模型充分改善时提前停止', '对于噪声很大的数据，优先选择少量互补模型，避免候选库过杂'],
    performance: {
      speed: { level: 'low', description: '串行且每轮需训练多个候选模型，速度较慢' },
      accuracy: { level: 'medium-high', description: '异构残差信号有效时可能提升；短验证段与噪声也可能导致退化' },
      dataRequirement: { level: 'medium', description: '需要足够的数据形成时间顺序滚动OOF样本以评估残差' },
  ...
  },
  {
    ...
    icon: Layers,
    category: '模型融合方法',
    summary: 'Stacking（堆叠法）构建多层学习架构：第一层多个异构模型生成最多三折扩展窗口滚动起点折外预测，第二层用合并OOF矩阵拟合无截距非负最小二乘（NNLS）元模型。',
    principle: {
      description: 'Stacking的哲学是“让模型教模型”。不同模型有不同的视角和偏好——ARIMA刻画差分后序列的线性自相关，趋势可通过差分或漂移项表达；LSTM则可表示非线性时序关系。Stacking训练元模型组合这些输出，并用多个时间因果滚动起点上的折外预测降低单次切分偶然性。',
      keyIdea: '用机器学习的方式学习如何做机器学习融合。本系统自动生成扩展窗口Level-0前缀和互不重叠的Level-1折，让元模型基于未参与对应基础模型训练的OOF预测来学习组合系数。'
    },
    implementationNotes: [
      '常见 Stacking 实现会用交叉验证预测训练元模型；时间序列数据不能随意打乱做普通K折。',
      '本系统按时间顺序生成最多三组扩展窗口 Level-0/Level-1：基础模型在每折前缀训练，对紧随其后的互不重叠区间预测，再将全部OOF样本合并学习组合；样本不足时退化为单折。',
      '引导训练把每个“基础模型×Level-1折”作为独立检查点；只有全部有效折完成后才拼接OOF矩阵Z并拟合一次NNLS元模型。',
      ...
      '成员和最终销量预测做非负兜底；不确定性只用合并Level-1 OOF组合残差校准成员逐horizon增长形状，独立评估真实值不参与校准。'
    ],
    mathematics: {
    ...
    workflow: {
      steps: [
        { title: '数据划分', description: '按时间顺序动态生成最多三折扩展窗口Level-0/Level-1，单折Level-1至少为K+2个点。', tip: '验证段互不重叠；不足时减少折数，再不足则明确拒绝训练' },
        { title: 'Level-1 OOF预测', description: '逐折用Level-0前缀训练所有基础模型，并预测紧随其后的Level-1区间。', tip: '合并后的OOF预测构成元模型训练特征' },
        { title: '训练元模型', description: '以合并Level-1 OOF真实值为目标求解非负无截距最小二乘。', tip: '保留原始系数，不归一化也不回退另一算法' },
    ...
    parameters: [
      ...
      { name: '动态滚动划分', description: '扩展窗口Level-0/Level-1折', impact: '单折Level-1至少为成员数+2个点，同时必须保留所有成员可训练的Level-0；最多三折且Level-1互不重叠', typical: '单折目标为max(K+2, ceil(20%N))；若早期Level-0不可训练则减少折数' }
    ],
    suitability: {
      suitable: ['希望学习二层融合并检验精度潜力', '有多个表现不错但风格不同的基础模型', '数据量充足（支持时间顺序滚动OOF）', '有充足计算资源'],
    ...
    pros: [
      { title: '精度潜力', description: '在基础模型互补且Level-1 OOF样本充足时可能提高精度，但仍需独立评估确认' },
    ...
    cons: [
      { title: '数据饥渴', description: '需要额外的数据形成时间顺序OOF预测，小数据不友好' },
    ...
    bestPractices: ['优先选择原理差异较大的模型（如MA与LSTM），并用留出结果确认是否互补', '数据量少时优先使用加权平均；仅在Level-0可训练且Level-1至少保留K+2个点时比较Stacking', '若某成员系数为0，只能说明它在当前Level-1的非负线性组合中未获得正系数', '系统只继承MA窗口、ES α、ARIMA d与LSTM特征/数值缩放方式；ARIMA阶数和LSTM隐藏配置会在内部训练段重算。基础配置变化后应重新训练融合模型'],
    performance: {
