...
const ensembleModels: EnsembleModel[] = [
  {
    ...
    icon: Scale,
    category: '模型融合方法',
    summary: '加权平均融合用基础模型在内部时间验证段的残差MSE倒数形成候选权重，再按验证点数决定的启发式系数向等权组合收缩；最终预测是成员预测的固定加权和。',
    principle: {
    ...
    implementationNotes: [
      ...
      '先按带数据尺度ε的MSE倒数归一化，再按验证点数决定的启发式系数向等权组合收缩，降低短验证段产生极端权重的风险。',
      '内部验证只继承MA窗口、ES α、ARIMA d和LSTM历史特征/数值缩放方式；ARIMA阶数与LSTM隐藏参数按当前前缀重新推导，LSTM始终不接收已知未来特征。',
      '权重非负且和为1，但本实现不估计成员误差协方差，因此不是完整的最小方差组合。',
      '名义95%范围与名义99%上侧误差复用计算权重的同一个时间留出段的一次固定预测原点跨horizon组合残差，并继承成员逐horizon增长形状；它们明确标记为未校准、无覆盖率保证，独立评估真实值不参与估计。销量区间限制为非负并保证包含点预测。'
    ],
    mathematics: {
      ...
      variables: [
        ...
        { symbol: 'ρ', meaning: '由验证点数决定的启发式收缩系数；不是经过统计估计的可靠度' },
    ...
    workflow: {
      steps: [
        ...
        { title: '计算权重', description: '先计算误差倒数候选权重，再按验证点数决定的启发式系数向等权收缩。', tip: '最终权重之和为1' },
  ...
  },
  {
    ...
    implementationNotes: [
      ...
      '名义95%范围与名义99%上侧误差复用同一个Boosting选模时间留出段的一次固定预测原点跨horizon组合残差，并据此估计成员增长形状；它们明确标记为未校准、无覆盖率保证，独立评估真实值不参与估计。销量区间同步限制为非负并保证包含点预测。'
    ],
    mathematics: {
  ...
  },
  {
    ...
    implementationNotes: [
      ...
      'Level-1至少需要基础模型数+2个点；这只是NNLS的最低可运行门槛，不代表统计样本已经充足。不足时明确拒绝训练，不以inverse-MAE冒充Stacking。',
      '成员、最终销量预测和销量区间做非负兜底。名义误差范围复用拟合元模型的同一个Level-1留出段的一次固定预测原点跨horizon组合残差，明确标记为未校准、无覆盖率保证；独立评估真实值不参与估计。'
    ],
    mathematics: {
