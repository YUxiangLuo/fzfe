...
const GradeWeights: React.FC = () => {
  ...
  const handleSaveDetails = () => {
    const total = Object.values(tempProcessDetails).reduce((sum, val) => sum + val, 0);
    if (total !== 100) {
      alert('明细权重总和必须为100%');
      return;
    }
  ...
  const totalDetailWeight = Object.values(tempProcessDetails).reduce((sum, val) => sum + val, 0);
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-900">成绩组成权重</h2>
                ...
                <div className="space-y-3">
                  <div className="flex items-center space-x-3">
                    <input
                      ...
                      onChange={(e) => handleWeightChange(item.key as keyof Weights, parseInt(e.target.value))}
                    ...
                    <div className="flex items-center space-x-2">
                      <input
                        ...
                        max="100"
                        value={tempWeights[item.key as keyof Weights]}
                        onChange={(e) => handleWeightChange(item.key as keyof Weights, parseInt(e.target.value) || 0)}
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div
                      ...
                      style={{ width: `${tempWeights[item.key as keyof Weights]}%` }}
                    ></div>
                  </div>
                  
                  <div className="text-center">
                    <span className={`text-xl font-bold ${getWeightColor(tempWeights[item.key as keyof Weights])}`}>
                ...
                <div className="space-y-3">
                  <div className="flex items-center space-x-3">
                    <input
                      ...
                      min="0"
                      max="100"
                      value={tempProcessDetails[item.key as keyof typeof tempProcessDetails]}
                      onChange={(e) => handleProcessDetailChange(item.key, parseInt(e.target.value))}
                      className="flex-1 h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
                    />
                    <div className="flex items-center space-x-2">
                      <input
                        ...
                        max="100"
                        value={tempProcessDetails[item.key as keyof typeof tempProcessDetails]}
                        onChange={(e) => handleProcessDetailChange(item.key, parseInt(e.target.value) || 0)}
                        className="w-16 px-2 py-1 text-center border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      />
                      <span className="text-sm text-gray-600">%</span>
                    </div>
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div
                      ...
                      style={{ width: `${tempProcessDetails[item.key as keyof typeof tempProcessDetails]}%` }}
                    ></div>
                  </div>
                  
                  <div className="text-center">
                    <span className={`text-lg font-bold ${getWeightColor(tempProcessDetails[item.key as keyof typeof tempProcessDetails])}`}>
                      {tempProcessDetails[item.key as keyof typeof tempProcessDetails]}%
                    </span>
                  </div>
                </div>
              </div>
